...
use B3N\Azure\Typo3\Domain\Repository\IndexRepository;
...
class AzureIndexCommandController extends CommandController
{
    ...
    public function indexerCommand()
    {
        ...
        //var_dump($tree->getTree(1));

        //var_dump($tree->getDataInit(4));

    }

    public function __indexerCommand()
    {
        ...
        while (($index = $db->sql_fetch_assoc($databaseResource))) {

            $this->output->outputLine("Updating Azure Search index" . PHP_EOL);

            $whereStatement = 'pages.deleted = 0 AND pages.pid = ' . $index['pid'];
            $databaseResource = $db->exec_SELECTquery(
                '*',
                'pages',
                $whereStatement,
